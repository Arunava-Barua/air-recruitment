...
const AIR_CONFIG = {
  apiKey: "T8LQ2NKvO6s1FRBnEPWqhR1rlccg6aAVQW1a0kWf",
  issuerDid: "did:air:id:test:4P6SFHkZJafvP3CALQwADe5HdX8CaeRWuUGNU5SKtp",
  partnerId: "67264f2e-99a9-4ee0-b23b-6a0f96bb573a",
  credentialId: "c21hi0g0df71j00d7084Rb", // You may need to update this with your actual credential ID
};
...
export default function CandidatePage() {
  ...
  const handleAIRCredentialIssuance = async (
    requestData: any
  ): Promise<boolean> => {
    setIsIssuing(true);
    ...
    try {
      ...
      const credentialSubject = {
        ...
        Salary: parseInt(
          requestData.details.salary?.replace(/[^0-9]/g, "") || "50000"
        ), // Extract number from salary range
        Experience: parseInt(
          requestData.details.startDate
            ? Math.floor(
                (new Date().getTime() -
                  new Date(requestData.details.startDate).getTime()) /
                  (1000 * 60 * 60 * 24 * 365)
              ).toString()
            : "2"
        ), // Calculate years of experience
        "Crypto-Experience": 2, // Assuming candidate is a crypto user since they have a wallet
      };
      ...
      return new Promise((resolve) => {
        ...
        widgetRef.current.on("issueCompleted", () => {
          ...
          toast({
            ...
            description:
              "Credential has been issued to your wallet via AIR Kit",
          });
          resolve(true);
        });
        ...
        widgetRef.current.on("error", (error: any) => {
          ...
          toast({
            ...
            description:
              "An error occurred during credential issuance. Please try again.",
            variant: "destructive",
          });
          resolve(false);
        });
        ...
        try {
          ...
          setTimeout(() => {
            const iframes = document.querySelectorAll(
              'iframe[src*="air"], iframe[src*="credential"]'
            );
            iframes.forEach((iframe) => {
              ...
              try {
                ...
                console.log(
                  "Cross-origin iframe detected, using external styling approach"
                );
              }
            });
      ...
      setIssuanceError(
        err instanceof Error
          ? err.message
          : "An error occurred during credential issuance"
      );
      setIsIssuing(false);
      toast({
        title: "Issuance Error",
        description:
          "Failed to start credential issuance process. Please try again.",
        variant: "destructive",
      });
      return false;
    }
  ...
  const handleCredentialRequest = () => {
    ...
    toast({
      ...
      description: `Request sent to ${
        organizations.find((org) => org.id.toString() === selectedOrg)?.name
      }`,
    });
                ...
                {issuanceSuccess && (
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    ...
                    <p className="text-green-600 text-sm">
                      The credential has been added to your wallet and is now
            ...
            <CardContent className="p-6">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
                <div className="text-xs text-gray-700 space-y-1">
                  <p>
                    <strong>Issuer DID:</strong> {AIR_CONFIG.issuerDid}
                  </p>
                  <p>
                    <strong>Partner ID:</strong> {AIR_CONFIG.partnerId}
                  </p>
                  <p>
                    <strong>Credential ID:</strong> {AIR_CONFIG.credentialId}
                  </p>
                  <p>
                    <strong>API Key:</strong>{" "}
                    {AIR_CONFIG.apiKey.substring(0, 8)}...
                    {AIR_CONFIG.apiKey.substring(AIR_CONFIG.apiKey.length - 4)}
                  </p>
                  <p>
                    <strong>Environment:</strong> {environmentConfig.apiUrl}
                  </p>
                </div>
              </div>
            </CardContent>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <p className="text-gray-600 mb-2">
                            From: <strong>TechCorp Inc.</strong>{" "}
                            {/* Replace with actual employer name */}
                          </p>
              ...
              {incomingRequests.filter((req) => req.status === "pending")
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p className="text-sm mt-2">
                    Refresh the page to check for new requests
                  </p>
                </div>
              )}
            </CardContent>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="flex items-center justify-between mt-4">
                          ...
                          <div className="flex space-x-2">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handlePresentCredential(credential.id)
                              }
                            >
                            ...
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                handleDownloadCredential(credential.id)
                              }
                            >
            ...
            <div className="space-y-8">
              ...
              <Card className="shadow-sm border-0">
                <CardHeader>
                  <CardTitle className="flex items-center">
                    <Plus className="w-5 h-5 mr-2 text-blue-600" />➕ Request
                    Credential
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
